...
for appliance_name in APPLIANCE_CONFIG.keys():
	...
	# if not appliance_name in ['washing_machine', 'fridge', 'dishwasher']:
	# if not appliance_name in ['washing_machine', 'dishwasher']:
		continue
	...
	for index, model in enumerate(list_model):
		...
		checkpointer = ModelCheckpoint(filepath="model_" + appliance_name + '_' +  str(index) + '_' + str(num_epochs) + 'epo.hdf5', verbose=1, save_best_only=True)
